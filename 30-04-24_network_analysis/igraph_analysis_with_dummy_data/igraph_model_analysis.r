...
proteins <- read.csv("./igraph_analysis_with_dummy_data/protein_list.csv", header=TRUE)

interactions <- read.csv("./igraph_analysis_with_dummy_data/protein_interactions.csv")
...
## adding weights to each node from interactions file
E(g)$weight = interactions$expression
...
dcm
write.csv(dcm, "./igraph_analysis_with_dummy_data/centrality_measures.csv",row.names=FALSE)
